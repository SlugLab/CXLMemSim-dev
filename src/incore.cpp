...
Incore::Incore(const pid_t pid, const int cpu, struct PerfConfig *perf_config)
    : perf_config(perf_config) {
  /* reset all pmc values */
  this->perf[0] =
      init_incore_perf(pid, cpu, perf_config->cpu_ldm_stalling_config, 0);
  this->perf[1] =
      init_incore_perf(pid, cpu, perf_config->cpu_llc_writeback_config, 0);
  this->perf[2] = init_incore_perf(pid, cpu, perf_config->all_dram_rds_config,
                                   perf_config->all_dram_rds_config1);
  this->perf[3] =
      init_incore_perf(pid, cpu, perf_config->cpu_llcl_hits_config, 0);
}
...
bool get_cpu_info(struct CPUInfo *cpu_info) {
  ...
  LOG(DEBUG) << fmt::format(
      "MAX_CPUID={}, CPUFAMILY={}, CPUMODEL={}, CPUSTEPPING={}\n",
      cpu_info->max_cpuid, cpu_info->cpu_family, cpu_info->cpu_model,
